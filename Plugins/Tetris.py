...
class BootPlugin(Plugin):
    ...
    def run(self):
        ...
        def paintLanding(alreadyPainted):
            """
        ...
        def paintFullRow(alreadyPainted):
            """
        ...
        def game_place_nextTetromino(tetromino_xPos, tetromino_yPos, tetromino, nextTetromino, tetromino_number, nextTetromino_number):
            """
        ...
        def game_start(tetromino, nextTetromino, tetromino_number, nextTetromino_number, tetromino_xPos, tetromino_yPos):
            """
        ...
        def game_restart(event,game_run, game_level, game_lines, game_speed, pixelBoard, alreadyPainted, game_board):
        ...
        def landing(game_lines, game_speed, game_level):
            """
        ...
        def rMove(event, tetromino_xPos):
            if game_run:
                if check_rMove():
        ...
        def lMove(event, tetromino_xPos):
            if game_run:
                if check_lMove():
                    tetromino_xPos = tetromino_xPos - 1
                    tetromino_rewrite()
        ...
        def dMove(event,tetromino_yPos):
            if game_run:
                if check_dMove():
        ...
        def cwRotation(event,tetromino):
            if game_run:
                if check_cwRotation():
                    tetromino = list(reversed(list(zip(*tetromino))))
                    tetromino_rewrite()
        ...
        def ccwRotation(event,tetromino):
            if game_run:
